...
# Directories containing multiple runs (txt files) for each baseline method
CLAUDE_RESULTS_DIR = BASE_DIR / "LLM responses" / "claude"
OPENAI_RESULTS_DIR = BASE_DIR / "LLM responses" / "openai"

# Manually specified average accuracy for our method (0–1)
OURS_AVG_ACCURACY = 0.50
...
def visualize_accuracy_boxplot(model_run_metrics: Dict[str, List[Dict[str, Any]]],
    ...
    plt.figure()
    plt.boxplot(data, tick_labels=model_names)
    plt.ylabel("Accuracy (%)")
    plt.ylim(0, 100)
...
if __name__ == "__main__":
    ...
    openai_multi = evaluate_multiple_runs(openai_all_data, response_key="openai_response")
    ...
    # ---- Per-method bar graph (Claude & OpenAI only) ----
    ...
    # ---- Combined bar chart of average accuracies (Claude, OpenAI, + manual ours) ----
    model_avg_metrics = {
        "claude": claude_multi["avg_accuracy"],
        "openai": openai_multi["avg_accuracy"],
        "team 20 model": OURS_AVG_ACCURACY,  # manually specified
    }
    avg_plot_path = views_dir / "average_accuracy_with_ours.png"
    visualize_average_accuracies(model_avg_metrics, save_path=avg_plot_path)

    # ---- Box-and-whisker plot of distributions across runs (Claude & OpenAI only) ----
    model_run_metrics = {
        "claude": claude_multi["per_run"],
        "openai": openai_multi["per_run"],
    }
    boxplot_path = views_dir / "accuracy_boxplot_baselines_only.png"
