...
def generate_rotated_images(path, outdir="renders", n=6):
    ...
    for i in range(n):
        theta = 2 * np.pi * (i / n)
        cam_pose = trimesh.transformations.look_at_matrix(
            eye=[2*np.cos(theta), 0, 2*np.sin(theta)],
            target=[0, 0, 0],
            up=[0, 1, 0]
        )
        ...
        Image.fromarray(color).save(f"{outdir}/view_{i:03d}.png")
        scene.remove_node(scene.get_nodes(camera)[0])
...
def solve_problem(problem_image_path, cutout_image_path):
    ...
    response = send_to_claude(prompt, image_paths=image_paths, model_id="claude-sonnet-4-5")
    return response

    

#solve_problem("./screenshots/rotations/1.png", "./screenshots/cropped/1.png")
generate_rotated_images("./tmp/preview_model.glb")
